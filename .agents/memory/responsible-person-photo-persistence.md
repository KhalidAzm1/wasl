...
**How to apply:** For every record containing both user-editable metadata and a stored-file reference, use the server's existing path as the fallback during list replacement and return a newly signed display URL on reads.